/// <summary>
/// Auto generated offsets for PUBG 
/// Extracted from https://github.com/TJ888/PUBG-SDK
/// </summary>

namespace Offsets
{
	public class UNewWorldMapWidget_C
	{
		///<summary><![CDATA[FPointerToUberGraphFrame]]></summary>
		public const int UberGraphFrame = 0x0290;

		///<summary><![CDATA[UWidgetAnimation *]]></summary>
		public const int MapFadeOut = 0x0298;

		///<summary><![CDATA[UWidgetAnimation *]]></summary>
		public const int MapFadeIn = 0x02A0;

		///<summary><![CDATA[UButton *]]></summary>
		public const int Button_2 = 0x02A8;

		///<summary><![CDATA[UWidgetSwitcher *]]></summary>
		public const int HelpWidget = 0x02B0;

		///<summary><![CDATA[UWidgetSwitcher *]]></summary>
		public const int HelpWidgetXboxSwitcher = 0x02B8;

		///<summary><![CDATA[UImage *]]></summary>
		public const int Image_1 = 0x02C0;

		///<summary><![CDATA[UImage *]]></summary>
		public const int Image_2 = 0x02C8;

		///<summary><![CDATA[UImage *]]></summary>
		public const int Image_3 = 0x02D0;

		///<summary><![CDATA[UImage *]]></summary>
		public const int Image_4 = 0x02D8;

		///<summary><![CDATA[UImage *]]></summary>
		public const int Image_5 = 0x02E0;

		///<summary><![CDATA[UImage *]]></summary>
		public const int Image_6 = 0x02E8;

		///<summary><![CDATA[UImage *]]></summary>
		public const int Image_7 = 0x02F0;

		///<summary><![CDATA[UImage *]]></summary>
		public const int Image_8 = 0x02F8;

		///<summary><![CDATA[UImage *]]></summary>
		public const int Image_9 = 0x0300;

		///<summary><![CDATA[UImage *]]></summary>
		public const int Image_10 = 0x0308;

		///<summary><![CDATA[UImage *]]></summary>
		public const int Image_11 = 0x0310;

		///<summary><![CDATA[UImage *]]></summary>
		public const int Image_12 = 0x0318;

		///<summary><![CDATA[UImage *]]></summary>
		public const int Image_13 = 0x0320;

		///<summary><![CDATA[UImage *]]></summary>
		public const int Image_14 = 0x0328;

		///<summary><![CDATA[UImage *]]></summary>
		public const int Image_15 = 0x0330;

		///<summary><![CDATA[UImage *]]></summary>
		public const int Image_16 = 0x0338;

		///<summary><![CDATA[UMapGridWidget_C *]]></summary>
		public const int MapWidget = 0x0340;

		///<summary><![CDATA[UButton *]]></summary>
		public const int MinimizeHelpButton = 0x0348;

		///<summary><![CDATA[UImage *]]></summary>
		public const int PadGuideImg = 0x0350;

		///<summary><![CDATA[UButton *]]></summary>
		public const int RestoreHelpButton = 0x0358;

		///<summary><![CDATA[UCurveFloat *]]></summary>
		public const int MapSizeCurve = 0x0360;

		///<summary><![CDATA[UCurveFloat *]]></summary>
		public const int MapSizeChangeTimeCurve = 0x0368;

		///<summary><![CDATA[FVector2D]]></summary>
		public const int MouseCapturePosotion = 0x0370;

		///<summary><![CDATA[bool]]></summary>
		public const int bMouseDown = 0x0378;

		///<summary><![CDATA[unsigned char[0x3]]]></summary>
		public const int UnknownData00 = 0x0379;

		///<summary><![CDATA[float]]></summary>
		public const int Time_MapSize = 0x037C;

		///<summary><![CDATA[bool]]></summary>
		public const int bChangeMapSize = 0x0380;

		///<summary><![CDATA[unsigned char[0x3]]]></summary>
		public const int UnknownData01 = 0x0381;

		///<summary><![CDATA[FVector2D]]></summary>
		public const int StartSize = 0x0384;

		///<summary><![CDATA[FVector2D]]></summary>
		public const int SizeOffset = 0x038C;

		///<summary><![CDATA[unsigned char[0x4]]]></summary>
		public const int UnknownData02 = 0x0394;

		///<summary><![CDATA[UCurveFloat *]]></summary>
		public const int PositionMoveTimeCurve = 0x0398;

		///<summary><![CDATA[float]]></summary>
		public const int Time_Position = 0x03A0;

		///<summary><![CDATA[bool]]></summary>
		public const int bMovePosition = 0x03A4;

		///<summary><![CDATA[unsigned char[0x3]]]></summary>
		public const int UnknownData03 = 0x03A5;

		///<summary><![CDATA[FVector2D]]></summary>
		public const int StartPosition = 0x03A8;

		///<summary><![CDATA[FVector2D]]></summary>
		public const int PositionOffset = 0x03B0;

		///<summary><![CDATA[FVector2D]]></summary>
		public const int TestMove = 0x03B8;

		///<summary><![CDATA[bool]]></summary>
		public const int WheelUp = 0x03C0;

		///<summary><![CDATA[unsigned char[0x7]]]></summary>
		public const int UnknownData04 = 0x03C1;

		///<summary><![CDATA[TArray<FVector2D>]]></summary>
		public const int AlignmentStack = 0x03C8;

		///<summary><![CDATA[float]]></summary>
		public const int LastZoomLevel = 0x03D8;

		///<summary><![CDATA[FVector2D]]></summary>
		public const int NewVar_1 = 0x03DC;

		///<summary><![CDATA[FVector2D]]></summary>
		public const int MouseDownPosition = 0x03E4;

		///<summary><![CDATA[bool]]></summary>
		public const int NewVar_2 = 0x03EC;

		///<summary><![CDATA[unsigned char[0x3]]]></summary>
		public const int UnknownData05 = 0x03ED;

		///<summary><![CDATA[int]]></summary>
		public const int NewVar_3 = 0x03F0;

		///<summary><![CDATA[bool]]></summary>
		public const int bMapMovingReady = 0x03F4;

		///<summary><![CDATA[bool]]></summary>
		public const int bIsReplaying = 0x03F5;

		///<summary><![CDATA[unsigned char[0x2]]]></summary>
		public const int UnknownData06 = 0x03F6;

		///<summary><![CDATA[FScriptMulticastDelegate]]></summary>
		public const int OnHideMyself = 0x03F8;

		///<summary><![CDATA[UCurveFloat *]]></summary>
		public const int ZoomMultiplier = 0x0408;

	}

}
