/// <summary>
/// Auto generated offsets for PUBG 
/// Extracted from https://github.com/TJ888/PUBG-SDK
/// </summary>

namespace Offsets
{
	public class UTslItemDragWidget_C
	{
		///<summary>FPointerToUberGraphFrame</summary>
		public const int UberGraphFrame = 0x0240;

		///<summary>UImage *</summary>
		public const int Background = 0x0248;

		///<summary>UImage *</summary>
		public const int ErrorIcon = 0x0250;

		///<summary>UImage *</summary>
		public const int ItemIcon = 0x0258;

		///<summary>UVerticalBox *</summary>
		public const int StackcountBlock = 0x0260;

		///<summary>UTextBlock *</summary>
		public const int StackCountText = 0x0268;

		///<summary>TScriptInterface&lt;USlotInterface&gt;</summary>
		public const int ItemInterface = 0x0270;

		///<summary>UItem *</summary>
		public const int ItemInstance = 0x0280;

		///<summary>ATslPlayerController *</summary>
		public const int ShooterPlayerController = 0x0288;

		///<summary>AInventory *</summary>
		public const int Inventory = 0x0290;

		///<summary>UMaterialInstanceDynamic *</summary>
		public const int BackgroundMaterial = 0x0298;

		///<summary>FName</summary>
		public const int ActionName = 0x02A0;

		///<summary>bool</summary>
		public const int bEnableAction = 0x02A8;

		///<summary>bool</summary>
		public const int bVicinityItem = 0x02A9;

		///<summary>unsigned char[0x2]</summary>
		public const int UnknownData00 = 0x02AA;

		///<summary>int</summary>
		public const int MouseOverWeaponSlotIndex = 0x02AC;

		///<summary>int</summary>
		public const int DragDroppingWeaponIndex = 0x02B0;

		///<summary>EEquipSlotID</summary>
		public const int DragDroppingSlotId = 0x02B4;

		///<summary>EEquipSlotID</summary>
		public const int MouseOverSlotId = 0x02B5;

	}

}
